...
##################     PROJECT: 1)    Cab Fare Prediction  Project       ##################   ##################   ##################

#Remove the elements
rm(list = ls())

#Set working directory
setwd("/home/nupur/Downloads/")
...
x = c("ggplot2", "corrgram", "DMwR", "usdm", "caret", "randomForest", "e1071",
      "DataCombine", "doSNOW", "inTrees", "rpart.plot", "rpart",'MASS','xgboost','stats','sp','raster')
...
# check for standard deviation for all variables after imputation
sapply(train, sd, na.rm = TRUE)
# fare_amount   pickup_datetime  pickup_longitude 
...
#Normality check
#Histogram helps to easily check normality of continuous predictor
hist(train$dist)#skewed
...
# mae       mse      rmse      mape 
# 1.9053850 6.3682283 2.5235349 0.2335395Improving Accuracy by using Ensemble technique ---- XGBOOST
...
write.csv(xgb_pred,"predictions_xgboost using R.csv",row.names = FALSE)
